...
router.put('/', async (req, res)=> {
    ...
    const data = await db.query("SELECT * FROM recipe WHERE recipename =$1;", [recipename]);
    if(data.rows.length===0) {
        res.json({message:"the is no such recipe"});}
        else {
            try {
...
router.post('/allingredientinrecipe', async (req, res)=> {
    const {recipename, ingredientname} = req.body;

    const data = await db.query('SELECT a.recipeName, b.ingredientName FROM recipe a INNER JOIN IngredientInRecipe c ON a.id = c.recipeId INNER JOIN ingridient b ON b.id = c.ingredientId WHERE a.recipeName = $1 AND b.ingredientName = $2;', [recipename, ingredientname]);

    if(data.rows.length!==0) {
        res.json({message:"record already exists"});}
        else {
            try {
                const result = await db.query("INSERT INTO ingredientinrecipe (recipeid, ingredientid) SELECT a.id, b.id FROM recipe a JOIN ingridient b ON a.recipeName = $1 AND b.ingredientname = $2;", [recipename, ingredientname]);
                res.status(200).json({message: `${result.rowCount} row was added.`});
            }
